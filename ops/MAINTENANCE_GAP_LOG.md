...
| `workspace-account-selection` | User-dependent authorization | Google Workspace connector is enabled, but its active account is empty; even a local CLI help invocation is blocked until one authorized account is selected. | Keep the read-only check opt-in; do not query, select, expose, or store an account automatically. |
